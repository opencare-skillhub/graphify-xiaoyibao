...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from xyb.detect import _load_graphifyignore, _is_ignored, _is_noise_dir
    ...
    for dirpath, dirnames, filenames in os.walk(target, followlinks=True):
        ...
        for fname in filenames:
            p = dp / fname
            if p.suffix in _EXTENSIONS and not fname.startswith(".") and not any(_is_noise_dir(part) for part in p.parts) and not _ignored(p):
                results.append(p)
    return sorted(results)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m xyb.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
